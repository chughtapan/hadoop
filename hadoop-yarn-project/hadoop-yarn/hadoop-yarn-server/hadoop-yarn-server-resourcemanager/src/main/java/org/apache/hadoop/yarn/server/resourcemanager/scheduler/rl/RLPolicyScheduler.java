...
public class RLPolicyScheduler extends
    AbstractYarnScheduler<FifoAppAttempt, FiCaSchedulerNode> implements
    Configurable {

  private static final Logger LOG =
      LoggerFactory.getLogger(RLPolicyScheduler.class);
  ...
  private final int action_space = (queue_size + 1);

  private static final String EOL = System.getProperty("line.separator");
  private String logFilePath;
  private BufferedWriter logFileBW = null;
  ...
  public RLPolicyScheduler() {
    super(RLPolicyScheduler.class.getName());
  }

  protected void setLogFilePath(String logFilePath) {
    LOG.info("Setting logFilePath=" + logFilePath);
    this.logFilePath = logFilePath;
    try {
      this.logFileBW = new BufferedWriter(new OutputStreamWriter(new FileOutputStream(this.logFilePath), "UTF-8"));
    } catch (IOException e) {
      e.printStackTrace();
    }
  }
  ...
  private synchronized void initScheduler(Configuration conf) {
    ...
    String policyModelPath = System.getenv("YARN_RL_POLICY");
    policyModel = Module.load(policyModelPath);
  }
  ...
  private void assignContainers(FiCaSchedulerNode node) {
    ...
    float[] feature_vector = new float[feature_count];
    List<ApplicationId> applicationIds = new ArrayList<ApplicationId>();
    List<SchedulerRequestKey> schedulerRequestKeys = new ArrayList<SchedulerRequestKey>();
    ...
    for (Map.Entry<ApplicationId, SchedulerApplication<FifoAppAttempt>> e : applications
      ...
      synchronized (application) {
        ...
        for (SchedulerRequestKey schedulerKey :
          ...
          int cores = pendingAsk.getPerAllocationResource().getVirtualCores();
          long memory = pendingAsk.getPerAllocationResource().getMemorySize();
    ...
    Tensor data =
            Tensor.fromBlob(feature_vector, new long[]{feature_count});
    IValue result = policyModel.forward(IValue.from(data));
    int action = (int) result.toTuple()[0].toTensor().getDataAsLongArray()[0];
    float log_prob = (float) result.toTuple()[1].toTensor().getDataAsFloatArray()[0];

    final String info = "feature_vector:" + Arrays.toString(feature_vector) + "\taction: " + action + "\tlog_prob: " + log_prob;
    LOG.info(info);
    if (logFileBW != null) {
      try {
        logFileBW.write(info + EOL);
        logFileBW.flush();
      } catch (IOException e) {
        e.printStackTrace();
      }
